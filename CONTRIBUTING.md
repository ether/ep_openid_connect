# Contributing to ep_openid_connect

Thanks for helping improve `ep_openid_connect` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_openid_connect` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_openid_connect from your local clone
pnpm run plugins i --path ../ep_openid_connect

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_openid_connect/` are picked up after a server restart.
...
* Etherpad version, ep_openid_connect version, browser + OS
* Steps to reproduce
